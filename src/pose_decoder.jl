...
function PoseDecoder(encoder_out_channels, n_input_features::Int, n_predictions::Int)
    squeezer = Conv((1, 1), encoder_out_channels=>256, relu; bias=false)
    pose = Chain(
        Conv((3, 3), (n_input_features * 256)=>256, relu; pad=1, bias=false),
        Conv((3, 3), 256=>256, relu; pad=1, bias=false))
    rotation = Conv((1, 1), 256=>(3 * n_predictions); bias=false)
    translation = Conv((1, 1), 256=>(3 * n_predictions); bias=false)
...
function (decoder::PoseDecoder)(features)
    ...
    # pose =  .* reshape(pose, (6, decoder.n_predictions, size(pose, 4)))
